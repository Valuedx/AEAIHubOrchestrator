...
| `TenantPolicyDialog.tsx` | **ADMIN-01** — per-tenant overrides for three operational knobs (execution quota / snapshot retention / MCP pool size). Per-field source pill (`override` vs. `env default`) + tri-state pending changes (unchanged / set / reset). |
