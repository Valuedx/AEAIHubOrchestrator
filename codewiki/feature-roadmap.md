...
| CTX-MGMT.C | Per-node `dependsOn` / `exposeAs` scope declaration. v1 ships exposeAs aliasing (engine writes `context[alias] = context[node_id]` so downstream Jinja can use `{{ case.id }}` instead of `{{ node_4r.json.id }}`) + validator collision check + lint extension. dependsOn is informational at v1; runtime scope enforcement deferred to v2 (per-render-site context filter). 16 new unit tests; 1229 backend tests pass. P1. | **Done (v1)** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.E | Fan-in primitive (Merge `waitAny` semantics now wired into the engine — the registry entry's strategy field finally honored), sub-workflow `_runtime.shared_evidence` promotion, and `lint_unreachable_node_after_switch` SMART-04 lint catching the V9-pre-fix bug shape. 23 new unit tests; 1213 backend tests pass. P1. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.G | ReAct iterations: split summary (default) vs full (opt-in `exposeFullIterations`). New helpers in `react_loop.py`; `iterations` keeps only action + tool names (no args / no results / no reasoning); `iterations_full` (scrubbed) emitted only when opted in. `predicates._all_tool_calls` reads new shape first, falls back to legacy. 16 new unit tests; 1190 backend tests pass. P1. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.I | `outputSchema` (JSON Schema) on every node — engine validates handler output at write-time. Soft by default (`_schema_mismatch` annotation + warn); strict-fail via `outputSchemaStrict: true`. Validator catches malformed schemas at promote time. New SMART-04 lint `jinja_ref_path_not_in_schema` extends the dangling-ref check with field-path awareness (e.g. typo `node_4r.id` when the handler produces `node_4r.json.id`). Latent fix in `extract_node_refs` — alias-based refs (`{{ case.foo }}`) now captured. 33 new unit tests; 1262 backend tests pass. P2. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
...
| CTX-MGMT.J | First-class `distillBlocks` for context distillation in LLM/ReAct prompts. Generalises V10's manual `RECENT TOOL FINDINGS` Jinja loop into declarative `[{label, fromPath, limit, project, format}]` config. New `app/engine/distill.py`; wired into `_handle_agent` and `run_react_loop` after `render_prompt`. Three formats: bullet/numbered/json. Validator catches typos at promote time. 40 new unit tests; 1302 backend tests pass. P2. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.F | Write-time secret scrub. Migration 0038: `tenant_policies.context_secret_scrub_enabled` DEFAULT TRUE. Engine runs the existing key-based `scrub_secrets` as the FIRST post-handler step (both sequential and parallel paths) so every downstream step sees the scrubbed value. Source-inspection regression test guards the ordering. 14 new unit tests; 1316 backend tests pass. P2. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.M | Forgetting/decay. Migration 0039: `tenant_policies.checkpoint_retention_days INTEGER NULL`. New `app/engine/forgetting.py` with per-node `retainOutputAfterRun` flag (clears slot at end-of-run before context_json persist; clears alias slot too) and `prune_aged_checkpoints` operator utility for retention sweep. Beat-task scheduling deferred — function is available manually today. 16 new unit tests; 1332 backend tests pass. P3. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.M2 | Beat-task wiring for `prune_aged_checkpoints` — daily sweep at 04:00 UTC (`crontab(hour=4, minute=0)` in `app/workers/scheduler.py`). Enumerates every tenant with checkpoints via `distinct(WorkflowInstance.tenant_id) JOIN InstanceCheckpoint`, calls the M v1 pruner per-tenant so each `checkpoint_retention_days` policy is honored independently. Per-tenant exception → log+rollback+continue (one bad tenant never breaks the daily sweep). 5 new unit tests; 1337 backend tests pass. Closes the v2 follow-up flagged in M's "what's NOT in v1" list. P3. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.J v2 | distillBlocks ride per-turn user message instead of system prompt. `assemble_agent_messages` gains `distill_text: str = ""` kwarg; `_handle_agent` and `run_react_loop` pass rendered distill through instead of concatenating into the system prompt. System message is now byte-stable across turns regardless of distill drift, so provider prefix caches (Anthropic/Vertex/OpenAI) actually hit. Source-inspection regression test guards both handler wires. 8 new unit tests including direct cache-stability proof; 1345 backend tests pass. Closes the v2 follow-up flagged in J's "what's NOT in v1" list. P2. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.H v2 | Context-trace reads + misses. New `record_read`/`record_miss`/`flush_render_events` helpers in `app/engine/context_trace.py`. `prompt_template.py` adds a thread-local capture wired into `_DotDict` and `_PermissiveUndefined`; runtime accumulates `(op, key)` tuples on `_runtime["_pending_render_events"]`. Runner flushes after `record_write` with 1-in-N read sampling (default 10, tunable) and never-sampled misses; consecutive duplicates collapse. 24 new unit tests; 1369 backend tests pass. Closes the v2 follow-up flagged in H v1. P2. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.C v2.a | Runtime `dependsOn` enforcement — Jinja-only filter (first of three v2 slices). New `app/engine/scope.py` with pure helpers + thread-local current-node stash (no race between sibling parallel branches). `prompt_template.render_prompt` filters safe_context to declared deps + aliases + infrastructure keys when `dependsOn` is set; existing graphs unaffected. `dag_runner` stashes `nodes_map` at all 4 entry points and sets per-thread current-node before each `dispatch_node`. `safe_eval` (v2.b) and structured-user-message-block (v2.c) deferred. 35 new unit tests; 1404 backend tests pass. P1. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.C v2.c | Runtime `dependsOn` enforcement — structured user-message bundle (second of three v2 slices). `build_structured_context_block` gains `node_data` + `nodes_map` kwargs with the same thread-local-stash fallback as `render_prompt`. Loop over `context.items()` skips `node_*` keys not in declared deps; trigger/loop_item/etc. always emitted. Aliases in dependsOn resolve to source node ids. Closes the half-enforcement gap from v2.a so both system prompt AND per-turn user message workflow context are scope-filtered. 12 new unit tests; 1416 backend tests pass. P1. | **Done** — see [context-management-plan.md §3](context-management-plan.md) |
| CTX-MGMT.C v2.b | Runtime `dependsOn` enforcement — `safe_eval` filter (third of three v2 slices). 12 call sites with non-uniform `eval_env` shapes; risk medium because Condition/Loop expressions return values that steer branches (wrong filter = routing bug, not a render-to-empty). **Explicitly dropped 2026-05-01** — risk concentrated, reward partial (prompt surface already covered by v2.a/v2.c), no production incident motivating it. Reactivation criteria recorded in plan doc §3.C v2.b. For now the existing `jinja_ref_outside_depends_on` lint catches authoring drift on safe_eval refs via the shared `extract_node_refs` regex. P2. | **Deferred** — see [context-management-plan.md §3.C v2.b](context-management-plan.md) |
