...
| `copilot_test_scenarios` | COPILOT-03.a — persisted regression scenarios the agent saves and re-runs; bound to either `draft_id` or `workflow_id` via an XOR check constraint (0027). COPILOT-V2 (0034) added `expected_predicates_json` for behavior-quality assertions alongside the original `expected_output_contains_json` partial-match dict | Yes |
...
| 384daed57459 | `384daed57459_fix_memory_embedding_dim_768.py` | **Embedding dim repin to 768** — repins `kb_chunks.embedding`, `embedding_cache.embedding`, `memory_records.embedding` from `vector(1536)` to `vector(768)` to match Vertex `text-embedding-005` / Google `gemini-embedding-2` (the new `embedding_default_*` settings). Drops + recreates the HNSW indexes at the new dimension. **Breaking for tenants on OpenAI 1536-dim**: pgvector cannot cast between dimensions, so the migration runs a pre-flight `vector_dims()` check and refuses to upgrade if any rows are at the wrong dim. Operator runbook: back up affected tables → `UPDATE … SET embedding = NULL` → run `alembic upgrade head` → re-embed via the running app (KB re-ingest or conversation replay). See the migration's docstring for the full procedure. |
| 0034 | `0034_copilot_scenario_predicates.py` | **COPILOT-V2** — add `expected_predicates_json JSONB NULL` to `copilot_test_scenarios`. Stores a list of `{type, args}` predicate assertions evaluated by `app/copilot/predicates.py` after `run_scenario`. Complements the existing `expected_output_contains_json` partial-match dict assertion — predicates are richer for behavior-quality checks (`ends_with_question`, `contains_any`, `lacks_terms`, `intent_in`, `regex_match`, `no_max_iterations_marker`, `tool_called`, `no_tool_called`, `tool_call_count`, `contains_all`). Backwards compatible: nullable, defaults to NULL; existing scenarios keep working unchanged. |
| 0035 | `0035_node_output_artifacts.py` | **CTX-MGMT.A** — new `node_output_artifacts` table for per-node overflow payloads. When a node's output exceeds the per-node `contextOutputBudget` (default 64 kB, hard ceiling 256 kB), the engine persists the full output here and replaces the in-context value with a small stub. Columns: `id UUID PK`, `tenant_id`, `instance_id` (FK to workflow_instances ON DELETE CASCADE), `node_id`, `output_json JSONB`, `size_bytes`, `budget_bytes`, `created_at`. Indexes: `(instance_id, node_id)` for the inspect-tool point lookup, `(tenant_id, created_at)` for retention sweeps. RLS tenant-scoped via the standard `app.tenant_id` GUC policy. |
| 0039 | `0039_tenant_policies_checkpoint_retention.py` | **CTX-MGMT.M** — adds `tenant_policies.checkpoint_retention_days INTEGER NULL`. NULL = use `SYSTEM_DEFAULT_RETENTION_DAYS` (30 days). Tenants needing shorter retention (cost) or longer (compliance) override here. Drives the `prune_aged_checkpoints` operator utility's per-tenant sweep on `InstanceCheckpoint` rows. Pairs with the new per-node `retainOutputAfterRun: bool` config flag — when False, the engine clears that node's slot from `context` at end-of-run before persisting `instance.context_json`. |
| 0038 | `0038_tenant_policies_secret_scrub.py` | **CTX-MGMT.F** — adds `tenant_policies.context_secret_scrub_enabled BOOLEAN NOT NULL DEFAULT TRUE`. Default-on (close-leak by default). Controls whether the engine runs the existing key-based `scrub_secrets` (from `app/engine/scrubber.py`) on `context[node_id] = output` writes. Already runs on log writes since 0001; this flag adds the in-memory write-path. Pure/functional — original handler `output` reference is unchanged; the in-context value is the scrubbed copy. |
| 0036 | `0036_instance_context_trace.py` | **CTX-MGMT.H** — new `instance_context_trace` table for per-instance context-write event log (v1: writes only; reads + misses deferred). Columns: `id UUID PK`, `tenant_id`, `instance_id` (FK to workflow_instances ON DELETE CASCADE), `node_id`, `op` (always `"write"` in v1; `"read"` / `"miss"` reserved), `key`, `size_bytes`, `reducer` (records the CTX-MGMT.L reducer applied), `overflowed BOOLEAN` (records whether the write was a CTX-MGMT.A overflow stub), `ts`. Indexes: `(instance_id, key, ts)` for the inspect-tool point lookup, `(tenant_id, ts)` for retention sweeps. RLS tenant-scoped. Migration also adds `tenant_policies.context_trace_enabled BOOLEAN NOT NULL DEFAULT FALSE` — opt-in for production tracing; ephemeral copilot-initiated instances always trace regardless of this flag. |
| 0037 | `0037_tenant_policies_compaction.py` | **CTX-MGMT.K** — adds `tenant_policies.context_compaction_enabled BOOLEAN NOT NULL DEFAULT TRUE`. Within-run compaction is a cost saver (smaller `context_json` + smaller InstanceCheckpoint rows + smaller LLM-prompt-rendered structured-context blocks); default-on for the common case. Tenants with strict audit-trail requirements can opt out. The full output remains available via `node_output_artifacts` (CTX-MGMT.A) regardless of this flag. Default opposite to `context_trace_enabled` (0036, default FALSE — opt-in observability). |
