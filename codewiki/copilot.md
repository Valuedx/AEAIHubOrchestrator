...
- `components/copilot/PromoteDialog.tsx` (02.ii) — confirmation modal for Apply. Shows node/edge counts with +/- deltas against the base workflow, surfaces the draft's validation (errors + lints block; warnings + lint-warnings pass through), collects an optional description (plus a required name for net-new drafts), and wires the Apply button to `api.promoteDraft` with `expected_version` for the optimistic-concurrency race guard. `max-w-2xl max-h-[85vh] flex flex-col` so the diff summary + name + validation list all fit a 1366×768 viewport without cramping.
...
- **`frontend/src/components/copilot/PromoteDialog.test.tsx`** (02.ii) — 5 dialog unit tests: validation errors block promote, warnings pass through, net-new drafts require a non-empty name, +/- deltas render against base workflow counts, confirm calls `api.promoteDraft` with the `expected_version` body shape and forwards the result.
...
- Full suite: **702 passed, 21 skipped** backend + **134 passed** frontend on this branch (up from 469 + 128 pre-COPILOT).
