...
| `search_docs(query, top_k?)` | Read | Runner | Two-mode docs search: (a) **word-overlap** (default, COPILOT-01b.iii) — precomputed token sets over `codewiki/*.md` + a flattened view of `shared/node_registry.json`, zero embedding cost; (b) **vector** (SMART-05, opt-in via `smart_05_vector_docs_enabled`) — the first call embeds the whole corpus (~30–50 chunks) via `embedding_provider.get_embeddings_batch_sync` (default `openai` / `text-embedding-3-small`), L2-normalises, caches in-process; each query embeds once and ranks by dot product. Envelope stamps `backend: "word_overlap" \| "vector"` so the agent can narrate how it searched. **Graceful degradation**: if the embedding provider is unreachable (no credential, network error), the vector path auto-falls back to word-overlap and adds `vector_fallback: "<reason>"` to the result. Cache invalidation: `docs_index.reset_cache()` drops the corpus text cache, `reset_vector_cache()` drops the embeddings (also invalidated automatically when the provider/model pair changes). |
...
- Full suite: **729 passed, 21 skipped** backend + **145 passed** frontend on this branch (up from 469 + 128 pre-COPILOT).
