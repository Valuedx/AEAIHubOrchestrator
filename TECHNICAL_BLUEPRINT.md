> - **SMART-02 — per-tenant accepted-patterns library (2026-04-22)**: Third SMART-XX ticket and the one that turns the copilot from a generic builder into a tenant-conditioned one. Every successful `/promote` persists a snapshot of the accepted graph into a new `copilot_accepted_patterns` table (migration `0026`, tenant-scoped RLS). Along with the graph we store the *originating NL intent* — the first `user` turn of the draft's most recent session, pulled via `_first_user_intent`. This is load-bearing: the intent → graph pair is the few-shot the retrieval path serves back on future drafts. Manual-edit promotes (no session) still save the pattern but leave `nl_intent` null; retrieval then weights them as low-score untagged candidates. Denormalisation decisions: `node_types` (JSONB array of distinct registry labels) and `tags` (JSONB array of category + label tokens) are extracted at save time so the retrieval hot path doesn't re-walk the graph on every query. `node_count` and `edge_count` are stored too for display. Index `ix_accepted_pattern_tenant_created (tenant_id, created_at)` drives the "top 50 candidates per tenant" fetch in `recall_patterns`; we then in-memory-score with word-overlap (reuses the same tokenise + stopword logic shape as `docs_index.py`) with a 2× title boost. `SMART_02_RETRIEVAL_CANDIDATES = 50` and `SMART_02_RECALL_MAX_TOP_K = 10` constants keep the per-query cost bounded for any tenant volume. Save is best-effort: the promote endpoint calls `save_accepted_pattern` inside the same transaction but the save function swallows exceptions (logs warning, returns None) so a rare FK violation or connection blip never poisons a promote. Retrieval tool `recall_patterns(query, top_k?)` takes the user's free-form intent and returns ranked candidates with full `graph_json` so the agent can directly adapt. Returns `{enabled: false, patterns: []}` when the tenant opts out via `smart_02_pattern_library_enabled`. Per-tenant flag added to the same migration (`0026`, default TRUE because save + retrieve are pure DB I/O — no LLM, no embeddings). System prompt's phase 3 rewritten: `recall_patterns` now runs BEFORE any `add_node` call on a non-trivial intent; the agent is instructed to adapt the closest high-score pattern before falling back to canonical shapes (classifier+router, RAG-over-KB, ReAct-with-MCP). Tool definitions, runner_tools dispatch, `CopilotToolName` union, `TenantPolicyOut.flags`, `TenantPolicyUpdate`, and `EffectivePolicy` all updated following the now-established SMART-XX template. UI summary in `CopilotToolResultCard` shows "N patterns" or italic "disabled" on the collapsed card and the query preview in the tool-call pill. 20 new tests: 13 in `test_copilot_pattern_library.py` covering tokeniser + graph extraction + save opt-out + save-on-exception-swallow + recall scoring + title boost + zero-score filter + top_k clamp + retrieval flag-off + graph_json round-trip, plus 7 fixture updates across `test_copilot_runner_tools.py` / `test_tenant_policy_resolver.py` / `test_tenant_policies_api.py` for the new field and 1 new UI test. Backend 648 passed / 21 skipped (up from 625); frontend 123 (up from 122).
>
> - **SMART-06 — MCP tool discovery for the copilot (2026-04-22)**: Second SMART-XX ticket and first demonstration that the flag template scales — one migration column (`0025`), one `_pick_bool` line in the resolver, one entry in `TenantPolicyOut.flags`, one `CopilotToolName` union entry, one runner tool (`discover_mcp_tools`), done. The tool itself wraps `engine.mcp_client.list_tools` which already TTL-caches per `(tenant_id, pool_key)` for 5 minutes via MCP-02, so discovery is effectively free after the first call per session. Design decision: no new cache layer here — the existing MCP cache is warm across agent turns, and a second cache would just invalidate itself when the first one expires. Result shape normalises the MCP tool payload: `{name, title, description, category, safety_tier, tags}` with sensible fallbacks for optional fields. The agent's system prompt gains a bullet telling it to call `discover_mcp_tools` early in sessions where the intent involves external API calls and to propose matching MCP Tool nodes rather than re-implementing the call as HTTP. When `discovery_enabled: false` (tenant opted out), the narration is instructed to skip MCP tool suggestions entirely — don't nag a tenant who's turned the feature off. Unreachable-MCP degrade path: `list_tools` is wrapped in try/except so a network-level failure returns `{discovery_enabled: true, tools: [], error: "..."}` rather than raising through the agent's turn. The alternative would be surfacing the crash as a tool-layer error, but that's confusing UX — "discovery failed" is fundamentally different from "the agent called a broken tool." UI: `CopilotToolResultCard`'s per-tool summary gains a `discover_mcp_tools` case that shows "2 MCP tools" (enabled) or italic "disabled" (opted out) on the collapsed card. 7 new backend tests (5 tool-layer — enabled/disabled/label-passthrough/bad-arg/unreachable-MCP/dispatch-routing/name-membership — plus 2 resolver + API fixture updates for the new field) and 1 new UI test. Total backend 632 passed, 21 skipped (up from 625); frontend 122 passed (up from 121).
>
> - **SMART-04 — proactive authoring lints + per-tenant flag template (2026-04-22)**: First of the six-ticket SMART-XX intelligence-upgrade series (SMART-01..06 on the roadmap). Two shippable components in this slice: (1) the lint rules + agent integration, and (2) the per-tenant-feature-flag template that every future SMART ticket will copy. The lints: four O(nodes + edges) graph-structure checks in `app/copilot/lints.py` — `no_trigger` fires when a graph has nodes but no category=trigger entry (error; workflow has no way to start); `disconnected_node` fires when a non-trigger node can't be reached from any trigger via directed edges (warn; BFS from triggers, ignores orphan edges so they don't hide real disconnections); `orphan_edge` fires when an edge references a missing node_id (error; can happen after external graph edits or race conditions); `missing_credential` fires when an LLM-family node (LLM Agent, ReAct Agent, LLM Router, Reflection, Intent Classifier) has a provider configured but the ADMIN-03 `get_credentials_status` resolver returns `not_configured` (error; deliberately skipped for provider=vertex since that uses ADC, not a per-request key). Each lint is structured `{code, severity, node_id, message, fix_hint}` so the UI can render a per-severity colored card with a clickable fix hint, rather than a flat warning string. The composer function `run_lints(graph, tenant_id, db)` orchestrates all four and returns a de-duplicated list. New runner tool `check_draft` wraps `tool_layer.validate_graph` + `run_lints` in a single call — the agent's system prompt now directs it here instead of `validate_graph` (pure function kept for API-layer callers). The flag template: migration `0024` adds `smart_04_lints_enabled BOOLEAN NOT NULL DEFAULT TRUE` to `tenant_policies`; `EffectivePolicy` gains a boolean field + resolver gains a `_pick_bool` helper symmetric with the existing `_pick` integer helper; `TenantPolicyUpdate` accepts a nullable bool with the standard "null clears the override" semantic; `TenantPolicyOut` grows a new `flags: dict[str, bool]` field separate from `values: dict[str, int]` so the frontend can render typed toggles without schema-switching per key. Every future SMART-XX ticket adds one named boolean column + one `flags` entry + one `_pick_bool` line — the template is deliberately boring so the ticket-per-feature sprint pattern doesn't accumulate drift. Settings gains a corresponding `smart_04_lints_enabled: bool = True` so operators can flip the process-wide default via `ORCHESTRATOR_SMART_04_LINTS_ENABLED`. When the flag is false, `check_draft` skips `run_lints` entirely and returns `{errors, warnings, lints: [], lints_enabled: false}`; schema validation still runs. Exception safety: if `run_lints` itself raises (bug in a rule), `check_draft` catches, logs, and returns `{..., lints: [], lint_runtime_error: str}` so a broken lint can't poison the agent's turn. Frontend: `CopilotLint` + `CopilotLintSeverity` types, `check_draft` in the `CopilotToolName` union, `TenantPolicyOut.flags` typed, and `CopilotToolResultCard` renders a summary line on the collapsed card ("1 lint error, 2 lint warnings" with per-severity colour) + a full lint list in the expanded drawer where each lint is its own card with severity pill, code label, node id, message, and a "Fix: …" line below the message. The lints-disabled state surfaces a subtle italic note so users know whether the clean result is "clean" or "lints turned off". 23 new tests total: 18 lint-rule unit tests (every rule gets a fires-when-expected + silent-when-ok test, plus the credential rule has the `db=None` safety path and the Vertex skip); 5 `check_draft` integration tests including a patched `run_lints` failure to verify the degrade path; 3 new UI tests for the count summary, expanded card fields, and the lints-disabled hint. Backend 625 passed / 21 skipped (up from 602); frontend 121 passed (up from 118). Design rule for SMART-XX feature flags: each is a per-tenant opt-out with a design-default matching the table in `codewiki/feature-roadmap.md`; default ON for features with zero marginal LLM/embedding cost (SMART-04 lints, SMART-02 pattern retrieval, SMART-03 telemetry reads, SMART-06 MCP tool discovery) and default OFF for features that incur net-new cost (SMART-01 regression re-runs, SMART-05 embedding indexing).
>
> - **COPILOT-02.i — chat pane + streaming message list + composer (2026-04-22)**: First user-facing surface for the copilot. The backend work from 01a (draft workspace + pure tool layer), 01b.i (Anthropic agent runner + SSE streaming), 01b.ii.a (test_node), 01b.ii.b (execute_draft + get_execution_logs), 01b.iii (docs grounding), and 01b.iv Google/Vertex shipped without a UI and were technically unreachable by end users — this slice closes that gap. Four new frontend components under `frontend/src/components/copilot/`: (1) `CopilotPanel.tsx` is the right-side drawer container. Owns the ensure-draft-and-session bootstrap on open — `createDraft({ base_workflow_id })` if a workflow is loaded (so Promote lands as a new version) or a blank draft otherwise, then `createCopilotSession({ draft_id })`. Mounts the message list + composer. Wires an `AbortController` through `api.sendCopilotTurn` so closing the panel mid-stream cancels the fetch cleanly. Has three display states — `loading` / `error` (with retry) / `ready` — so the user never sees a blank drawer during bootstrap. (2) `CopilotMessageList.tsx` is a scrollable list with auto-stick-to-bottom; when the user scrolls up past an 80-px threshold the sticky flips off and a "Jump to latest" pill appears in the corner. Dotted "Copilot is working…" indicator renders while a turn is streaming. Empty state surfaces three concrete prompt examples so the user isn't staring at a blank chat. (3) `CopilotComposer.tsx` is an auto-growing textarea — 1 row minimum, 12 rows maximum (clamped by computed line-height so huge pastes don't eat the canvas). Cmd/Ctrl+Enter sends; plain Enter inserts a newline. Disabled while streaming — cancel-a-turn is deferred to 02.ii alongside an explicit stop button. (4) `CopilotToolResultCard.tsx` is the event-stream dispatcher. Exports `CopilotEventCard` which switches on `CopilotAgentEvent.type`: `assistant_text` → left-aligned prose bubble (max-w-[95%], relaxed leading, whitespace-pre-wrap), `tool_call` → compact pill "🔧 name — summary" with the arg summary chosen per-tool (`add_node` → node_type, `connect_nodes` → "src → tgt", `search_docs` → first 40 chars of the query, etc.) and click-to-expand JSON, `tool_result` → colour-coded card (green success / red error) with per-tool result summary ("added node_7", "0 errors · 1 warnings", "completed (412 ms)", "timeout", "7 matches"), expand to reveal full result JSON + validation warnings list + draft version. `done` events render null (they're just stream terminators). `error` events render in an amber banner (recoverable) or red (not). Layout integration in App.tsx: swaps PropertyInspector for CopilotPanel when the toolbar dispatches a `copilot:toggle` window event — mutual exclusivity rather than stacking because both occupying the right column would leave the canvas at <600 px on a 1440 p display. Panel width 460 px is deliberate and load-bearing: this is the user's "panels should be large enough and visible" feedback codified — 288 px is what PropertyInspector uses for form fields, chat needs substantially more for prose + code blocks + JSON previews. Toolbar integration: Sparkles icon in the top bar between the existing separator and the FlaskConical Playground icon. Frontend async-generator contract from 01b.i is unchanged; the streaming-fetch + SSE frame parser handles abort signals, malformed JSON (emits a recoverable error event instead of killing the stream), and EOF. 10 new tests in `CopilotToolResultCard.test.tsx` covering the discriminated dispatch + per-tool summary strings — these are the bits users read at a glance and are easy to regress when a new tool lands. Full frontend suite: 118 passed (up from 108); zero backend changes required. Remaining scope explicitly deferred to 02.ii: `DraftDiffOverlay` painting copilot-added nodes with a sparkle badge on the canvas, `PromoteDialog` showing a side-by-side of draft vs. base with accept/reject, a stop-generating button, and replay of prior-session chat history via `listCopilotTurns`.
>
> - **COPILOT-01b.iii — docs grounding via `search_docs` + `get_node_examples` (2026-04-22)**: Pragmatic docs grounding for the copilot. Rather than routing through the existing pgvector RAG pipeline with a dedicated system knowledge base (which would need: a reserved tenant sentinel, RLS policy carveouts so cross-tenant reads work, a per-tenant embedding provider contract, a reindex-on-deploy CLI, and a migration for the sentinel tenant row), this slice ships a **file-backed word-overlap index** in `app/copilot/docs_index.py`. The index walks `codewiki/*.md` (19 files today) on first call, chunks each file at H2/H3 heading boundaries (intro-before-H2 becomes a synthetic "Overview" chunk), then flattens `shared/node_registry.json` into one chunk per node type plus a categories-index chunk. Chunk-level fields: `source_path`, `title`, `anchor` (GitHub slug for a future UI deep-link), `text` (capped at ~4 KB so a huge section doesn't blow the tool_result payload), `tokens` (precomputed lowercased token set, stopwords stripped), `kind` (`codewiki` | `registry` | `registry-index`), `node_type` (set on registry chunks only). Scoring: lowercase + regex-tokenise query, set-intersect with chunk tokens, small 2× boost for matches in the title; stopwords excluded from both sides; titles count even when the body has zero overlap (for phrases like "llm agent" where the section title is the canonical lookup). Ranking returns the top-k (default 5, capped at 20) sorted by score descending. Two agent-facing tools: `search_docs(query, top_k?)` for free-form concept lookups; `get_node_examples(node_type)` keys on an exact registry type id and returns `{registry_entry, related_sections}` — the related-search strips the registry chunk itself so the agent doesn't see the same content twice. Index is cached in a module-level global; `docs_index.reset_cache()` forces a reload (used by tests and after local doc edits). System prompt updated with a "ground in docs before drafting anything non-trivial" bullet plus coaching to rephrase when match counts are low (word-overlap has no synonyms). Design trade-off — the non-vector approach doesn't do semantic matching ("classifying the incoming messages" won't find the Intent Classifier section unless the docs use that exact phrasing) but a vector upgrade is a clean replacement of `docs_index.py`'s internals without any change to the tool surface or frontend types. No migration, no env vars, no service deps — the `codewiki/` directory is resolved via `app.paths.CODEWIKI_DIR` which points at `PROJECT_ROOT/codewiki`. 22 new tests covering index load + cache, chunker shape, anchor slug validity, search ranking (including title-boost behaviour + top_k clamping), `get_node_examples` with the known `automation_edge`/`automationedge.md` type-vs-filename mismatch, and dispatch routing. Full backend suite: 602 passed, 21 skipped (up from 580).
>
> - **COPILOT-01b.ii.b — `execute_draft` + `get_execution_logs` runner tools (2026-04-22)**: Second runner-tool slice, closing the construct → test → debug → fix loop for the copilot. Ships four things: (1) migration `0023` adds `is_ephemeral BOOLEAN NOT NULL DEFAULT FALSE` to `workflow_definitions`, non-breaking for existing rows; (2) filter sweep — `list_workflows` in `api/workflows.py` now filters `is_ephemeral=False` (user-facing saved-workflows dialog must not surface copilot temp rows), `scheduler.check_scheduled_workflows` in `workers/scheduler.py` adds the same filter so ephemerals never fire schedules, and `a2a.agent_card` pins the filter belt-and-suspenders on top of its existing `is_published=True` so a future bug that flips is_published on an ephemeral doesn't leak into a tenant's public A2A surface — deliberately the engine paths (`dag_runner`, node handlers, instance creation) do NOT filter on `is_ephemeral` because they need to load ephemeral WDs to actually run them; (3) `runner_tools.execute_draft_sync` which validates the draft via `tool_layer.validate_graph` (short-circuits on hard errors before materialising anything), creates an ephemeral `WorkflowDefinition` with name prefix `__copilot_draft_<draft-id>_<unix-ts>__` + `is_active=False` (defensive), creates a real `WorkflowInstance` pointing at it, commits those rows, then dispatches `execute_graph` in a `ThreadPoolExecutor.submit(...).result(timeout=N)` — same threadpool+asyncio-wait_for shape the live `POST /api/v1/workflows/{id}/execute?sync=true` endpoint uses. Timeout is clamped server-side to `[1, 300]` seconds so a pathological `timeout_seconds=9999` can't block an agent turn indefinitely. Three terminal outcomes: **completed** (return `{instance_id, status, elapsed_ms, output, started_at, completed_at}` with internal `_`-prefixed keys stripped from output per the sync-execute convention), **timeout** (engine keeps running in the background thread — no way to safely cancel in Python — return `{instance_id, status: "timeout", hint}` pointing the agent at `get_execution_logs` to poll), **failed** (engine raised; return `{instance_id, status: "failed", error}`); (4) `runner_tools.get_execution_logs` reads per-node logs via `ExecutionLog` filtered by `instance_id` — the safety boundary here is that the function first verifies the parent `WorkflowDefinition.is_ephemeral=True`, so arbitrary `instance_id` values from production runs are refused with a deliberately vague "not a copilot-initiated run" message (don't leak whether the id exists). Returns `{instance_id, status, log_count, logs: [{node_id, node_type, status, output_json, error, started_at, completed_at}]}`; `input_json` intentionally omitted to keep payload small for the LLM's context — the agent already knows what it sent. `cleanup_ephemeral_workflows(db, older_than_seconds=7*86400)` is the reaper utility operators call (or Beat calls, follow-up — not wired in this slice). Cascade deletes on `WorkflowInstance` + `ExecutionLog` take care of child rows. Frontend `CopilotToolName` union extended with `execute_draft` + `get_execution_logs` so COPILOT-02's chat pane can render the tool_result events with specialised UI (e.g. "Trial run completed (instance_id: …, 1,234ms)" + a "View logs" button that calls the agent back with `get_execution_logs`). 13 new tests (31 total in runner-tools suite); full backend 580 passed, 21 skipped (up from 567).
>
> - **COPILOT — AutomationEdge handoff fork (2026-04-22)**: Clarifies the design boundary between this orchestrator (AI-native DAG authoring) and AutomationEdge (deterministic RPA authoring) inside the copilot's system prompt and toolchain. The copilot recognises deterministic-automation intent (SAP postings, form fills, file moves, ERP updates, etc.) and, rather than trying to synthesise RPA steps with LLM nodes, explicitly offers the user two paths: (1) **inline** — add an `automationedge` node here pointing at an existing AE workflow, with the workflow name filled in by the user; or (2) **handoff** — open the AutomationEdge Copilot (separate product — explicitly out of this orchestrator's scope) to design the RPA steps, then return with a workflow name to use path (1). Same rule applies inside Sub-Workflows: if the sub-workflow is entirely deterministic, an `automationedge` node is cleaner than a full child graph. Implementation: new read-only runner tool `get_automationedge_handoff_info` that reads `tenant_integrations(system='automationedge')` rows and returns their connection summaries (label / base_url / org_code / is_default) plus each connection's optional `config_json.copilotUrl`. URL precedence: default connection's `copilotUrl` → any connection's `copilotUrl` → new `settings.ae_copilot_url` env default → `None`. Zero schema migration — `config_json` is already free-form JSONB, so the new `copilotUrl` field is a pure addition to the existing per-connection dict. The new `ORCHESTRATOR_AE_COPILOT_URL` env var gives a process-wide fallback for tenants that haven't set it per-connection yet. The `CopilotToolName` frontend union is extended with `get_automationedge_handoff_info` so COPILOT-02's chat pane can render the deep-link URL as a clickable button in the tool_result event. 6 new tests (full list: connections+default copilot URL, env fallback, default-connection precedence, empty tenant → null URL, dispatch routing, name set membership). 567 total backend passing (up from 561). Docs updated: `codewiki/copilot.md` §3 now shows 10 tools with a Pure/Runner family column; `codewiki/automationedge.md` §2c documents the new `copilotUrl` field in the Integrations dialog and the fork the copilot applies at authoring time.
>
> - **COPILOT-01b.ii.a — `test_node` runner tool (2026-04-22)**: Opens the stateful-tool family for the copilot. Design separates two tool families — **pure** (`app/copilot/tool_layer.py`: graph dict in, graph dict out, no DB) and **runner** (`app/copilot/runner_tools.py`: stateful, requires DB + tenant, invokes node handlers). This split is load-bearing: the agent runner in 01b.i chains many pure-tool calls in-memory per LLM turn and commits once; if runner tools were in the same module they'd either force per-call commits (messy) or break the "tool functions are side-effect-free" invariant. Agent's `_dispatch_tool` tries pure by name first, then runner (`RUNNER_TOOL_NAMES`) — any tool name in neither set returns an "Unknown tool" error back to the LLM. `test_node_against_draft` reuses the DV-02 synthetic-context recipe verbatim: walk every node with a `pinnedOutput`, merge an override dict from `args.pins` (takes precedence — lets the LLM probe "what if upstream returned X?" without editing the draft), attach `args.trigger_payload` to `context["trigger"]`, set synthetic `_instance_id` / `_current_node_id` / `_workflow_def_id=draft:<uuid>` for handlers that read those directly, then call `dispatch_node(node_data, context, tenant_id, db=db)`. Handler exceptions are caught inside `test_node_against_draft` and returned as `{node_id, error, elapsed_ms}` — LLM reads the message, suggests a fix, iterates. `NodeSuspendedAsync` (AutomationEdge-style) is caught specifically and returned with an explicit "expected side effect for this node type" string so the agent doesn't flag it as a failure. Runner tools don't mutate the draft graph, so `validation` in the tool_result event is always `null` and `draft_version` is unchanged. The runner-tool dispatch returns the top-level `"error"` key from the tool result as the agent's `dispatch_error`, which surfaces as `is_error=true` in the Anthropic tool_result block / `is_error` flag in the event stream — same self-correct loop the pure tool layer uses. Frontend `CopilotToolName` union extended with `test_node`. 14 new tests (12 runner-tool unit + 2 end-to-end through the agent loop). 561 total backend passing (up from 547). Deferred to 01b.ii.b: `execute_draft` + `get_execution_logs` — those need an `is_ephemeral BOOLEAN` column on `workflow_definitions` (migration 0023) plus filter sweeps in `list_workflows`, the scheduler's active-workflow query, and the A2A agent card's published-skills query, so the throwaway WorkflowDefinition rows `execute_draft` creates to satisfy WorkflowInstance's FK don't pollute the UI or get surfaced as A2A skills. That sweep is larger than `test_node` warranted on its own.
>
> - **COPILOT-01b.iv — Google AI Studio + Vertex AI providers (2026-04-22)**: Third shippable slice of COPILOT-01b. Extends the agent runner with Google AI Studio + Vertex AI support; pushes the OpenAI piece + per-session token budget to a follow-up since Vertex was the user-specified priority. Design: refactored `app/copilot/agent.py` from a hard-coded Anthropic flow into a provider-agnostic loop driven by a `_PROVIDER_ADAPTERS` dict. Each adapter entry is `{build_state, call, append_tool_round, initial_user_goes_in_state}` — the last flag distinguishes Anthropic (message list contains the new user message) from Google (call() takes `new_user_text` as a first-iteration kwarg since Google's `types.Content` history is assembled ahead of time and the live user turn is appended at call time). Google adapter: `_build_google_state` replays `copilot_turns` rows into `types.Content(role, parts)` objects — user→`user`, assistant→`model` with `Part.from_text` and `Part.from_function_call` parts, consecutive tool turns→batched `user` Content with `Part.from_function_response` parts (the per-turn storage format is provider-agnostic; only the replay differs). `_call_google` uses the unified `google-genai` SDK via `_google_client(backend, tenant_id)` from `llm_providers.py` — `backend="genai"` for AI Studio with `api_key` from ADMIN-03's `get_google_api_key`, `backend="vertex"` for Vertex AI with `vertexai=True` + per-tenant `project`/`location` from VERTEX-02's `_resolve_vertex_target`. Both google and vertex providers share a single adapter — only the backend string differs — so there's zero duplication between AI Studio and Vertex code paths. `tool_definitions.to_google_tools()` converts the hand-written JSON-schema tool definitions into `types.Tool(function_declarations=…)`; Google accepts the JSON Schema verbatim, so parameters pass through unchanged. Default model for both google and vertex is `gemini-3.1-pro-preview-customtools` — the preview variant Google ships specifically for agentic tool-calling workloads, chosen over the general Gemini 3.1 Pro Preview because the copilot's workload is entirely tool-driven and `-customtools` is explicitly tuned for that. Google doesn't emit per-call tool_use ids like Anthropic; the runner synthesises stable `gfn_<name>_<idx>` ids so tool-turn rows have a UI-renderable link back to their originating call (the LLM itself links tool_result to tool_use by function name, not id). Refactor preserved all 22 existing 01b.i tests (Anthropic path unchanged); 9 new tests in `test_copilot_agent_google.py` cover Gemini 3.x defaults, Vertex provider routing through the same adapter, state reconstruction from persisted turns, Google-specific iteration-cap behaviour. Total backend suite: 547 passed, 21 skipped (up from 538). OpenAI provider + per-session token budget columns + suspend-on-budget middleware remain for a follow-up.
>
