...
def fix_workflows():
    ...
    with engine.connect() as conn:
        res = conn.execute(text("SELECT id, name, version, graph_json FROM workflow_definitions")).fetchall()
        for wf_id, name, version, graph in res:
            changed = False
            ...
            for node in graph.get('nodes', []):
                ...
                if label == 'ReAct Agent':
                    if config.get('provider') == 'google':
                        ...
                        changed = True
                    # If tools are empty or missing, fill them
                    if not config.get('tools'):
                        config['tools'] = all_tools
                        changed = True
                    # Optional: even if not empty, if they are "routing" nodes, maybe they need all tools
                    # But let's stick to empty ones for now to be safe.
