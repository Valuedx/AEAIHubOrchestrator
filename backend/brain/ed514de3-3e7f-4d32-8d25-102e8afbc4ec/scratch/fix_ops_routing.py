    ...
    with engine.connect() as conn:
        ...
        if res:
            ...
            for node in graph.get('nodes', []):
                ...
                if label == 'ReAct Agent':
                    ...
                    if not config.get('tools'):
                        ...
                        changed = True
                    elif set(config.get('tools')) != set(all_tools):
                        config['tools'] = all_tools
                        changed = True
                
                # Also update LLM Router if it uses 'google'
                if label == 'LLM Router':
                    config = node.get('data', {}).get('config', {})
                    if config.get('provider') == 'google':
                        config['provider'] = 'vertex'
            ...
            if changed:
                ...
                conn.commit()
                print(f"AE_Ops_Routing updated successfully (ID: {wf_id})")
            else:
                print("No changes needed for AE_Ops_Routing")
        else:
