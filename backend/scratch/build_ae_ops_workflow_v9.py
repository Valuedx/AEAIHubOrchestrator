"""Build + POST AE Ops Support — V9 (router + worker + critic + smart HANDED_OFF).

V9 differs from V8 in one targeted way: the case-state Switch is moved
AFTER the intent router, and combines (case_state, intent) into the
routing decision. The motivating bug: in V8, once a case was HANDED_OFF
every subsequent message in the same session got the canned "logged on
case" reply — even if the user typed something completely new like
"restart timesheet workflow". V9 handles three sub-cases:

  HANDED_OFF + small_talk    → existing canned reply (covers "any update?",
                                "any news?", "did the team respond?")
  HANDED_OFF + cancel        → existing cancel branch (closes the
                                handed-off case via the same path
                                non-handed-off cases use)
  HANDED_OFF + ops/rca/handoff → NEW: archive the handed-off case +
                                  open a fresh one, then continue to
                                  worker. node_4r (HTTP node) calls
                                  /api/cases with archive_existing_handed_off=true
                                  so Flask atomically closes old + opens new.

All downstream nodes that previously referenced node_3.json.id now use
{{ node_4r.json.id if node_4r else node_3.json.id }} so the active case
flows downstream regardless of which arm fired.

router's small_talk intent gets extra training examples for status-check
phrasings ("any update?", "what's the status?") so they don't mis-route
to ops and trigger an unnecessary fresh-case path.
...
def build_graph() -> dict:
    ...
    # V9: moved EARLIER (was after the case-state Switch in V8). Glossary
    # only depends on the trigger message, so running it here lets the
    # state+intent decision below see both case state and intent in one
    # step.
    nodes.append(_node(
    ...
    nodes.append(_node(
        "node_router", "Intent Classifier",
        {
            ...
            "intents": [
                {
                    "name": "small_talk",
                    "description": "Greetings, thanks, jokes, 'what can you help with', off-topic, OR follow-up status checks on a case already with the team ('any update?', 'any news?', 'is it done yet?').",
                    "examples": [
                        # Greetings / chitchat
                        "hi", "hello", "thanks!", "thank you", "what can you help with",
                        "what do you do", "how are you", "good morning",
                        # V9: follow-up status-check phrasings. These typically
                        # arrive on a HANDED_OFF case where the user is asking
                        # whether the team has responded yet — the V9 decision
                        # routes small_talk + HANDED_OFF to the canned
                        # "logged on case" reply, which keeps the conversation
                        # on the existing case instead of opening a fresh one.
                        "any update?", "any updates?", "any news?",
                        "what's the status?", "what is the status?",
                        "is it done yet?", "is it resolved?",
                        "did the team respond?", "any progress?",
                        "have you heard back?", "still waiting?",
                    ],
                    "priority": 100,
                },
                {
    ...
    # 6b. node_4r — always-runs case-upsert (V9 NEW). This is a SECOND
    # /api/cases POST after node_router has classified the intent. The
    # Jinja-templated body asks Flask to archive_existing_handed_off
    # ONLY when the prior case is HANDED_OFF AND the user's intent
    # implies a NEW request (not a status check or cancel). Flask's
    # upsert is otherwise idempotent — on the active-state path this
    # call returns the same row as node_3.
    #
    # Why always-run: a single Switch deciding whether to archive
    # would produce a multi-source convergence on node_route, which
    # _propagate_edges prunes incorrectly (the default-arm edge's
    # _prune_subtree would kill node_route even though the
    # reset-case arm satisfies it). Always running node_4r keeps
    # node_route with a single predecessor so the prune logic is
    # straightforward; the cost is one extra ~10ms HTTP round-trip
    # per turn (negligible vs. LLM latency).
    nodes.append(_node(
    ...
    # 6b-i. Handed-off canned reply. Reached via node_route's
    # 'handoff_canned' synthetic case (set when node_4r returns a
    # still-HANDED_OFF case AND the intent is small_talk — the only
    # combination where archive doesn't fire and we want to stay on
    # the canned reply path).
    nodes.append(_node(
        "node_4d", "LLM Agent",
        {
            "icon": "user-x",
    ...
    # 7. Switch on (state + intent). V9: synthetic 'handoff_canned'
    # value covers HANDED_OFF case + small_talk intent — that's the
    # one combination where node_4r left the case state at HANDED_OFF
    # AND we want the canned "logged on case" reply rather than
    # routing to a specialist. Every other case is plain intent
    # routing because node_4r either left state intact (active path)
    # or archived + opened a fresh NEW case (HANDED_OFF + new
    # request).
    nodes.append(_node(
        "node_route", "Switch",
        {
    ...
    # V9: URL always uses node_4r (the always-runs upsert) which holds
    # the active case_id for this turn (either the same as node_3 or
    # the fresh one if archive fired).
    nodes.append(_node(
    ...
    # 7d. Cancel — close case + canned reply. V9: always use node_4r
    # which is the active case (matches node_3 on the cancel-on-active
    # path, and matches node_3 again on cancel-on-handed-off because
    # cancel intent skips the archive — both cases close the existing
    # row).
    nodes.append(_node(
        "node_cancel", "HTTP Request",
        {
            ...
            # V9 fix: Flask's /close endpoint expects resolution_summary,
            # not "reason" (inherited bug from V8 — its cancel branch
            # got 400s silently because the case-state Switch upstream
            # short-circuited before the /close call could fail
            # visibly).
...
def main() -> int:
    graph = build_graph()
    out_path = Path(__file__).parent / "ae_ops_support_workflow_v9.json"
    out_path.write_text(json.dumps(graph, indent=2))
    print(f"saved graph to {out_path}  ({len(graph['nodes'])} nodes, {len(graph['edges'])} edges)")

    payload = {
        "name": "AE Ops Support — V9 (smart HANDED_OFF)",
        "description": (
            "V9 = V8 + smart HANDED_OFF handling. The case-state Switch is "
            "moved AFTER the intent router; the routing decision combines "
            "(case state, intent) so a HANDED_OFF case does not absorb "
            "fresh ops requests on the same session. small_talk + HANDED_OFF "
            "still hits the canned 'logged on case' reply (covers 'any "
            "update?'); cancel + HANDED_OFF closes the existing case via the "
            "normal cancel branch; everything else on a HANDED_OFF case "
            "atomically archives the old case and opens a fresh one before "
            "continuing to the worker. Engine semantics, HITL gating, and "
            "verifier remain identical to V8."
        ),
        "graph_json": graph,
    }
